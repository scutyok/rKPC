...
fn get_swapchain_surface_format(formats: &[vk::SurfaceFormatKHR]) -> vk::SurfaceFormatKHR {
    // Prefer an sRGB-capable format when available so the final output
    // uses the correct color space/gamma. Fall back to the driver's
    ...
    // If an explicit SRGB format wasn't found, prefer any format that
    // reports an sRGB nonlinear color space.
    for f in formats {
    ...
    // Fallback to driver's preferred format.
    formats[0]
}
