...
pub struct EguiRenderer {
    ...
    // CPU-side staging arrays reused each frame to avoid allocations
    staging_vertices: Vec<EguiVertex>,
...
impl EguiRenderer {
    ...
    pub unsafe fn new(
        ...
        // Prepare CPU-side staging buffers (match GPU initial capacity)
        let staging_vertices: Vec<EguiVertex> = Vec::with_capacity(initial_vertex_capacity);
        let staging_indices: Vec<u32> = Vec::with_capacity(initial_index_capacity);
    ...
    pub unsafe fn render(
        ...
        // Collect vertices and indices into reusable staging arrays
        let vertices = &mut self.staging_vertices;
        let indices = &mut self.staging_indices;
        vertices.clear();
        indices.clear();
