//******************************************************************/
//
// Blood 2-style acceleration-based player movement.
//
// Ported from Lithtech Jupiter engine (`CMoveMgr::UpdateNormalMotion`) with Quake-style air-strafe physics.  
//
// Key features:
//
...
//******************************************************************/
//
// Movement tuning constants — derived from Blood 2 / Quake defaults, 
// scaled to the 0.01 Lithtech -> Vulkan coordinate system used by this renderer.
//
...
pub mod consts {
    pub const MAX_SPEED: f32 = 4.0; // Maximum ground speed the player can accelerate to (units/s).
    pub const GROUND_ACCEL: f32 = 50.0; // Ground acceleration (units/s²).
    pub const AIR_ACCEL: f32 = 8.0; // Air acceleration (units/s²) — low value gives Quake-style air-strafe.
    pub const FRICTION: f32 = 14.0; // Ground friction coefficient — higher = less slippery
    pub const GRAVITY: f32 = 22.0; // Gravity acceleration (units/s²).
    pub const JUMP_VELOCITY: f32 = 6.5; // Initial upward velocity when jumping (units/s).
    pub const TERMINAL_VELOCITY: f32 = 20.0; // Terminal falling speed (units/s).  Prevents tunnelling.
    pub const FLY_SPEED: f32 = 10.0; // Fly-mode (noclip) speed multiplier for direct velocity (units/s).
    pub const STOP_SPEED: f32 = 0.5; // Minimum speed threshold below which velocity is zeroed (prevents drift).
}

use consts::*;
pub type Vec2 = [f32; 2]; // XY velocity vector carried across frames.
...
pub struct MovementState {
    pub velocity: Vec2, // Horizontal velocity (X, Y) in world units/s.
    pub z_vel: f32, // Vertical velocity (Z) in world units/s.
    pub on_ground: bool, // True when the player is on a walkable surface.
    pub jumped: bool, // True the frame the player initiated a jump (prevents re-triggering).
}
...
pub struct MoveInput {
    pub wish_dir: [f32; 2], // Unit-length wish direction on the XY plane, or zero if no keys pressed.
    pub wish_move: bool, // Whether the player is pressing *any* horizontal movement key.
    pub jump: bool, // Jump requested this frame.
}
